...
class AppDrawer extends InheritedWidget {
                  ...
                  children: <Widget>[
                    Column(
                      ...
                      children: <Widget>[
                        ...
                        (ShrubsAppSession.of(context).userLoggedIn) ?
                          Text(ShrubsAppSession.of(context).user.mail, style: TextStyle(color: Color(0xFF757575), fontFamily: 'Roboto', fontSize: 14)) :
                          SizedBox(),
                      ],
                    ),
                    Spacer(),
  ...
  static List<Widget> items(context){
    return <Widget>[
              ...
              (ShrubsAppSession.of(context).userLoggedIn) ? AppDrawerItem('Profile', icon : Icon(OMIcons.person), route : ProfilePage.route) : SizedBox(),
              ...
              (ShrubsAppSession.of(context).userLoggedIn) ? AppDrawerItem('Orders', icon : Icon(OMIcons.redeem), route : OrdersPage.route) : SizedBox(),
              (ShrubsAppSession.of(context).userLoggedIn) ? AppDrawerItem('Favorites', icon : Icon(Icons.favorite_border), route : FavoriteProducts.route) : SizedBox(),
              AppDrawerItem('Notifications', icon : Icon(OMIcons.notifications), route : NotificationsPage.route),
              Divider(),
              AppDrawerItem('Help', icon : Icon(Icons.help), route : SupportPage.route),
    ];
  }
...
class SignOutButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      onTap: () {
        ShrubsAppSession.of(context).signOut(context);
      },
    );
  }
}
